...
import { TrendingDown, ShoppingBag, CreditCard, Users } from 'lucide-react';
...
export default function PurchaseReportsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await reportsAPI.getPurchases({ days });
        setData(res.data);
      } catch {
  ...
  const topVendorsColumns = [
    ...
    { key: 'orders', label: 'Total Orders', render: (val) => <span style={{ fontWeight: 600 }}>{val}</span> },
  ...
  return (
    <div className="animate-in">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div className="breadcrumb">Home / Reports / Purchases</div>
          <h1 className="page-title">Purchase Reports</h1>
          <p className="page-subtitle">Analyze your expenses and top vendors</p>
        </div>
        <div>
      ...
      <div className="dashboard-grid" style={{ marginBottom: '24px' }}>
        <div className="stat-card" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            ...
            <div className="stat-value" style={{ color: 'var(--danger)', marginTop: '8px' }}>₹{data.summary.total_expense.toFixed(2)}</div>
          </div>
        ...
        <div className="stat-card" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            <div className="stat-title">Top Vendors</div>
            <div className="stat-value" style={{ color: 'var(--info)', marginTop: '8px' }}>{data.top_vendors.length}</div>
          </div>
          <div style={{ backgroundColor: 'rgba(14, 165, 233, 0.1)', padding: '12px', borderRadius: '12px', color: 'var(--info)' }}>
            <Users size={24} />
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: '24px', marginBottom: '24px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.05), 0 2px 4px -2px rgb(0 0 0 / 0.05)' }}>
        <h3 style={{ marginBottom: '24px', fontSize: '1.25rem', fontWeight: 600 }}>Expense Trend</h3>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data.trend} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              ...
              <Tooltip 
                ...
                itemStyle={{ color: 'var(--danger)', fontWeight: 600 }}
                formatter={(value) => [`₹${value.toFixed(2)}`, 'Expense']} 
              />
              <Area type="monotone" dataKey="expense" stroke="var(--danger)" strokeWidth={3} fillOpacity={1} fill="url(#colorExpense)" activeDot={{ r: 8, strokeWidth: 0 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
